...
function getRoute(): NavKey {
  const value = window.location.hash.replace(/^#\/?/, '') as NavKey | 'pair';
  return navigation.some((item) => item.key === value) ? value as NavKey : 'chats';
}

function wantsPairing() {
  return window.location.hash.replace(/^#\/?/, '') === 'pair';
}
...
export default function App() {
  ...
  const [showPairing, setShowPairing] = useState(wantsPairing());
  const [welcomeStep, setWelcomeStep] = useState(0);

  useEffect(() => {
    const sync = () => { setRoute(getRoute()); setShowPairing(wantsPairing()); };
  ...
  const page = useMemo(() => {
    switch (route) {
      ...
      default: return <ChatsPage conversations={nexus.conversations} messages={nexus.messages} onSend={nexus.sendMessage} onPair={nexus.addContact} identity={nexus.identity} openPairing={showPairing} onPairingClose={() => { setShowPairing(false); window.location.hash = '#/chats'; }} />;
    }
  }, [nexus, route, showPairing]);
